...
function setProgress(active, ratio, text) {
...
function makeStatic(root) {
  root.traverse(o => {
    if (o.isObject3D) { o.matrixAutoUpdate = false; o.updateMatrix(); }
  });
  scene.updateMatrixWorld(true);
}

// Tighter frustum = less shaded stuff
function tightenFrustumTo(object) {
...
gltfLoader.load(
  ...
  /* onError */ (err) => { setProgress(false, 0, 'Error loading GLB'); console.error(err); }
);
