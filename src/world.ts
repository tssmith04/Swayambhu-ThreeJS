// [Author: Thomas Smith]
// World / Rendering / Controls / Movement
// [Author: leoata]
...
export function createWorld(app: HTMLElement): World {
  ...
  `;
    ...
    let worldOptions: WorldOptions | null = null;
    ...
    function animate() {
        ...
        if (opts && opts.isEducationalMode()) {
            // Update annotation visibility each frame
            opts.onUpdateAnnotations(player);
        }
        ...
        if (useDesktopControls || useMobileControls) {
            ...
            if (useDesktopControls) {
                ...
                else if (velocity.lengthSq() < 1e-4) movingStopSoon();
            } else {
                // On mobile, treat any joystick input as "moving"
                if (mobileMove.lengthSq() > 1e-4) movingStart();
                else if (velocity.lengthSq() < 1e-4) movingStopSoon();
            }
    ...
    function start(options: WorldOptions) {
